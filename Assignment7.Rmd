...
t <- ggplot(D3, aes(score)) + facet_wrap(~measure, scales = "free") 
t+ geom_histogram()
...
library(rpart)
c.tree1 <- rpart(level.up ~ post.test.score + pre.test.score + messages + forum.posts + av.assignment.score, method = "class", data = D1, control=rpart.control(minsplit=1, minbucket=1, cp=0.001))
printcp(c.tree1) # actually using av.assignment.score and post.test.score 
plot(c.tree1)
text(c.tree1)
# choose 2 variables
...
printcp(c.tree2)
post(c.tree2, file = "tree.ps", title = "level-up") 

#Generate a probability value that represents the probability that a student levels up based your classification tree 
D1$pred <- predict(c.tree1, type = "prob")[,2]
D1$pred1 <- predict(c.tree2, type = "prob")[,2]
...
pred1.detail <- prediction(D1$pred1, D1$level.up) 
plot(performance(pred1.detail, "tpr", "fpr"))
abline(0, 1, lty = 2)
unlist(slot(performance(pred1.detail,"auc"), "y.values")) # 0.9622563

# the first model was better, because it predict better since the true positive rate is closer to 1.
...
#Calculate kappa
kappa(matrix1, exact = TRUE)/kappa(matrix1) # 1.093018
```
...
kappa(matrix2, exact = TRUE)/kappa(matrix1) # 1.621722
