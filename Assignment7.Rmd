...
D2 <- gather(D1, "measure", "score", -id)
p1 <- ggplot(D2, aes(score)) + facet_wrap(~measure, scales = "free")+geom_histogram(stat = "count")
...
```
pre.test.score and id are strongly correlated.
#Classification tree
```{r}
...
D1$truepos.model1 <- ifelse(D1$level.up == "1" & D1$threshold.pred1 == "1", 1, 0)
D1$falsepos.model1 <- ifelse(D1$level.up == "0" & D1$threshold.pred1 == "1", 1,0)
D1$falseneg.model1 <- ifelse(D1$level.up == "1" & D1$threshold.pred1 == "0", 1,0)
...
D1$threshold.pred1 <- ifelse(D1$pred >= 0.9, 1, 0)
...
recall.model1 <- sum(D1$truepos.model1)/(sum(D1$truepos.model1) + sum(D1$falseneg.model1))
```
The higher the threshold value, the lower the accuracy and recall will be.
