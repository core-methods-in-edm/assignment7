...
library(dplyr)
library(tidyr)
DF1 <- read_csv('online.data.csv')
```

#Visualization 
```{r}
#Start by creating histograms of the distributions for all variables (#HINT: look up "facet" in the ggplot documentation)
#create dummy variable to show distribution of level up
DF2 <- select(DF1, 1:7)
DF2$level.up <- ifelse(DF1$level.up == "yes", 1,0)
DF3 <- gather(DF2, "measure", "scores", 2:7)
#Then visualize the relationships between variables
library(ggplot2)
p <- ggplot(data = DF3, aes(scores)) + geom_histogram(stat = "count")
p + facet_wrap(~measure, scales = "free")

#Try to capture an intution about the data and the relationships
# assignment score, messages, post test and pre test are more or less normal distributions, while the forum post skews left. Also, level up is not even but there are enough instances of yes to use the data. 
```
#Classification tree
```{r}
